...
void ruby_config::reg_options(class OptionParser * opp)
{
    ...
    option_parser_register(opp, "-ruby_cl_predictor_type", OPT_UINT32,
                           &m_cl_predictor_type,
                           "Cache Lease option - predictor type. 0=fixed, 1=global, 2=perL2, 3=PC-fixed, 4=PC-adaptive, 5=addr-fixed, 6=oracle, 7=PCsampler, 8=tardisdec, 9=tardisinc, 10=tardisPC-adaptive (default=0)",
                           "0");
    ...
    option_parser_register(opp, "-ruby_cl_pred_tardisdec", OPT_CSTR, &m_cl_pred_tardisdec_string,
                           "Minimum lease, maximum lease and shift bits for Tardis Decrement Predictor, shift right 1 bit just like divide 2 (min:max:shift) (default=8:64:1)",
                           "8:64:1" );
    option_parser_register(opp, "-ruby_cl_pred_tardisinc", OPT_CSTR, &m_cl_pred_tardisinc_string,
                           "Minimum lease, maximum lease and shift bits for Tardis Increment Predictor, shift left 1 bit just like multiply 2 (min:max:shift) (default=8:64:1)",
                           "8:64:1" );
    option_parser_register(opp, "-ruby_cl_pred_tardispcadaptive", OPT_CSTR, &m_cl_pred_tardispcadaptive_string,
                           "Minimum lease, maximum lease and shift bits for Tardis PC-Adaptive Predictor, shift left 1 bit just like multiply 2 (min:max:shift) (default=8:64:1)",
                           "8:64:1" );
    ...
    option_parser_register(opp, "-ruby_send_memfence_to_protocol", OPT_BOOL,
                           ...
                           "1");
    option_parser_register(opp, "-ruby_no_write_acks", OPT_BOOL,
                               &m_no_write_acks, "Disable write acknowledgements. Memory fences generate messages. (1=On, 0=Off (default))",
                               "0");

    option_parser_register(opp, "-ruby_writes_stall_at_mshr", OPT_BOOL,
                           &m_writes_stall_at_mshr, "Writes are stalled by pending requests in MSHRs (1=On, 0=Off (default) )",
                           "0");
    option_parser_register(opp, "-ruby_atomics_stall_at_mshr", OPT_BOOL,
                           &m_atomics_stall_at_mshr, "Atomics are stalled by pending requests in MSHRs (1=On, 0=Off (default) )",
                           "0");
    ...
    option_parser_register(opp, "-ruby_reads_stall_at_pending_writes", OPT_BOOL,
                               &m_reads_stall_at_pending_writes, "Reads stall if there is a pending write (1=On, 0=Off (default) )",
                               "0");
    ...
    option_parser_register(opp, "-ruby_request_buffer_size", OPT_UINT32,
                           &m_request_buffer_size, "Ruby request_buffer size (default=4096)",
                           "4096");
    option_parser_register(opp, "-ruby_request_buffer_enabled", OPT_BOOL,
                           &m_request_buffer_enabled, "Ruby request buffer enabled? (1=On, 0=Off (default) )",
                           "0");
    option_parser_register(opp, "-ruby_delay_queue_enabled", OPT_BOOL,
                           &m_delay_queue_enabled, "Delay queue for L2 response enabled? (1=On, 0=Off (default) )",
                           "0");
    option_parser_register(opp, "-ruby_delay_queue_t_per_cycle", OPT_UINT32,
                           &m_delay_queue_t_per_cycle, "Number of transitions per Ruby L1 delay queue per cycle (default=1)",
                           "1");
    option_parser_register(opp, "-ruby_delay_queue_insert_latency", OPT_UINT32,
                           &m_delay_queue_insert_latency, "Insert latency of Ruby L1 delay queue (default=1)",
                           "1");
    option_parser_register(opp, "-ruby_delay_queue_threshold", OPT_UINT32,
                           &m_delay_queue_threshold, "If response of L2 will increase pts of L1 more than this threshold, insert response into delay queue (default=10)",
                           "10");
    option_parser_register(opp, "-ruby_read_only_check", OPT_BOOL,
                           &m_read_only_check, "Distinguish read-only data and read/write data? (1=On, 0=Off (default) )",
                           "0");
    option_parser_register(opp, "-ruby_read_only_trace_file", OPT_CSTR, &m_read_only_trace_file,
                           "Filename for the memory trace (default=NULL)",
			   "NULL");
...
void ruby_config::init() {
   ...
   m_cl_pred_tardisdec_min = 0;
   m_cl_pred_tardisdec_max = 0;
   m_cl_pred_tardisdec_shift = 0;
   ntok = sscanf(m_cl_pred_tardisdec_string,"%u:%u:%u",
                        &m_cl_pred_tardisdec_min, &m_cl_pred_tardisdec_max, &m_cl_pred_tardisdec_shift);
   if( ntok < 3 ) {
       printf("GPGPU-Sim uArch: error while parsing configuration string ruby_cl_pred_tardisdec\n");
       abort();
   }

   m_cl_pred_tardisinc_min = 0;
   m_cl_pred_tardisinc_max = 0;
   m_cl_pred_tardisinc_shift = 0;
   ntok = sscanf(m_cl_pred_tardisinc_string,"%u:%u:%u",
                        &m_cl_pred_tardisinc_min, &m_cl_pred_tardisinc_max, &m_cl_pred_tardisinc_shift);
   if( ntok < 3 ) {
       printf("GPGPU-Sim uArch: error while parsing configuration string ruby_cl_pred_tardisinc\n");
       abort();
   }

   m_cl_pred_tardispcadaptive_min = 0;
   m_cl_pred_tardispcadaptive_max = 0;
   m_cl_pred_tardispcadaptive_shift = 0;
   ntok = sscanf(m_cl_pred_tardispcadaptive_string,"%u:%u:%u",
                        &m_cl_pred_tardispcadaptive_min, &m_cl_pred_tardispcadaptive_max, &m_cl_pred_tardispcadaptive_shift);
   if( ntok < 3 ) {
       printf("GPGPU-Sim uArch: error while parsing configuration string ruby_cl_pred_tardispcadaptive\n");
       abort();
   }
...
gpusim_ruby_config::gpusim_ruby_config(const gpgpu_sim_config &config) {
    ...
    m_no_write_acks = config.m_memory_config.m_ruby_config.m_no_write_acks;
    ...
    m_cl_pred_tardisdec_min = config.m_memory_config.m_ruby_config.m_cl_pred_tardisdec_min;
    m_cl_pred_tardisdec_max = config.m_memory_config.m_ruby_config.m_cl_pred_tardisdec_max;
    m_cl_pred_tardisdec_shift = config.m_memory_config.m_ruby_config.m_cl_pred_tardisdec_shift;
    m_cl_pred_tardisinc_min = config.m_memory_config.m_ruby_config.m_cl_pred_tardisinc_min;
    m_cl_pred_tardisinc_max = config.m_memory_config.m_ruby_config.m_cl_pred_tardisinc_max;
    m_cl_pred_tardisinc_shift = config.m_memory_config.m_ruby_config.m_cl_pred_tardisinc_shift;
    m_cl_pred_tardispcadaptive_min = config.m_memory_config.m_ruby_config.m_cl_pred_tardispcadaptive_min;
    m_cl_pred_tardispcadaptive_max = config.m_memory_config.m_ruby_config.m_cl_pred_tardispcadaptive_max;
    m_cl_pred_tardispcadaptive_shift = config.m_memory_config.m_ruby_config.m_cl_pred_tardispcadaptive_shift;
    ...
    m_request_recorder_enabled = config.m_memory_config.m_ruby_config.m_request_buffer_enabled;
    m_delay_queue_enabled = config.m_memory_config.m_ruby_config.m_delay_queue_enabled;
    m_delay_queue_t_per_cycle = config.m_memory_config.m_ruby_config.m_delay_queue_t_per_cycle;
    m_delay_queue_insert_latency = config.m_memory_config.m_ruby_config.m_delay_queue_insert_latency;
    m_delay_queue_threshold = config.m_memory_config.m_ruby_config.m_delay_queue_threshold;
    m_read_only_check = config.m_memory_config.m_ruby_config.m_read_only_check;
    m_read_only_trace_file = config.m_memory_config.m_ruby_config.m_read_only_trace_file;
}
...
ruby_wrapper::ruby_wrapper(const gpgpu_sim_config &g_config)
   : m_ruby( new gpusim_ruby(gpusim_ruby_config(g_config)) ),
     ...
     m_mshr_stall_cycles(5, "ruby_mshr_stall_cycles", 20, NULL)
{
   ...
   m_mshr_stalled.resize(g_config.num_shader(), false);
   m_mshr_stall_cycle.resize(g_config.num_shader(), 0);

   m_request_buffer_enabled = g_config.m_memory_config.m_ruby_config.m_request_buffer_enabled;
   m_request_buffer_size = g_config.m_memory_config.m_ruby_config.m_request_buffer_size;
   dynamic_request_id = 0;
   if (m_request_buffer_enabled) {
       FILE *outfile = fopen("mem_access_pattern.txt", "a+");
       fprintf(outfile, "%-10s%-6s%-6s%-6s%-10s%-14s%-18s%-18s%-18s%-10s%-30s\n", "req_id", "core", "cta", "warp", "req_type", "pc", "timestamp", "addr", "block_addr", "mshr_hit", "kernel");
       fflush(outfile);
       fclose(outfile);
   }
}
...
void ruby_wrapper::send_request( unsigned core_id, mem_fetch * mf, unsigned dynamic_cta_id, unsigned dynamic_warp_id, std::string kernel_name )
{
   ...
   request_buf_entry rbe;
   rbe.request_id = dynamic_request_id;
   rbe.core_id = core_id;
   rbe.addr = mf->get_addr();
   rbe.block_addr = block_addr;
   rbe.timestamp = gpu_sim_cycle + gpu_tot_sim_cycle;
   rbe.pc = mf->get_pc();
   rbe.req_type = type == ruby_load_type   ? "LOAD"   :
	          type == ruby_store_type  ? "STORE"  :
		  type == ruby_membar_type ? "MEMBAR" :
		  type == ruby_atomic_type ? "ATOMIC" : "";
   rbe.mshr_hit = "Miss";
   rbe.dynamic_cta_id = dynamic_cta_id;
   rbe.dynamic_warp_id = dynamic_warp_id;
   rbe.kernel_name = kernel_name;

   if(mf->is_membar()) {
      m_ruby->send_memfence_request( block_addr, mf->get_data_size(), core_id, mf->get_wid(), dynamic_cta_id, dynamic_warp_id, dynamic_request_id, mf->get_pc(), type, false, space, mf->get_byte_mask());
   } else {
      ...
      if(m_gpgpu_config.m_shader_config.gmem_skip_L1D and space==ruby_global_space) {
         ...
         m_ruby->send_request( block_addr, mf->get_data_size(), core_id, mf->get_wid(), dynamic_cta_id, dynamic_warp_id, dynamic_request_id, mf->get_pc(), type, false, space, mf->get_byte_mask(), mf);
         sent_to_ruby = true;
      } else if (is_write and !m_config.m_writes_stall_at_mshr) {
         // Skipping MSHRs for writes/atomics
         m_ruby->send_request( block_addr, mf->get_data_size(), core_id, mf->get_wid(), dynamic_cta_id, dynamic_warp_id, dynamic_request_id, mf->get_pc(), type, false, space, mf->get_byte_mask(), mf);
         sent_to_ruby = true;
         if(!m_config.m_no_write_acks) {
             // Tell MSHR about this write (if write acks are enabled)
   ...
   m_request_buffer.push(rbe);
   if (m_request_buffer.size() == m_request_buffer_size)
       dump_request_buffer();
...
bool ruby_wrapper::is_ready( unsigned core_id, mem_fetch * mf, unsigned dynamic_cta_id, unsigned dynamic_warp_id,  ruby_stall_type &m_ruby_stall_reason)
{
   bool isReady; 
   bool is_write = mf->get_is_write() or mf->isatomic(); // atomic requires modifying the cache line
   ...
   if(mf->is_membar()) {
       ...
       isReady =  m_ruby->isReady( block_addr, mf->get_data_size(), core_id, mf->get_wid(), dynamic_cta_id, dynamic_warp_id, dynamic_request_id, type, false, space);
       m_ruby_stall_reason = isReady ? NO_RUBY_STALL : RUBY_SEQUENCER_STALL;
       return isReady;
   }
   ...
   if(m_gpgpu_config.m_shader_config.gmem_skip_L1D && space==ruby_global_space) {
      ...
      if ( m_mshr[core_id]->probe(block_addr) ) {
         ...
         bool isAccessAllowed = m_mshr[core_id]->access_allowed(block_addr, is_write, mf, ruby_stall_reason);
         ...
         m_ruby_stall_reason = isReady ? NO_RUBY_STALL : isMSHRReady? RUBY_MSHR_ACCESS_NOT_ALLOWED : RUBY_MSHR_STALL;

      } else {
         // MSHR miss - check for downstream back pressure from ruby
         bool isRubyReady = m_ruby->isReady( block_addr, mf->get_data_size(), core_id, mf->get_wid(), dynamic_cta_id, dynamic_warp_id, dynamic_request_id, type, false, space);
         ...
         m_ruby_stall_reason = isReady ? NO_RUBY_STALL : isRubyReady ? RUBY_MSHR_STALL : RUBY_SEQUENCER_STALL;
      }
   }

   if (m_ruby_stall_reason != NO_RUBY_STALL) {
       if (!m_mshr_stalled[core_id]) {
           m_mshr_stall_cycle[core_id] = gpu_sim_cycle + gpu_tot_sim_cycle;
	   m_mshr_stalled[core_id] = true;
       }
   } else {
       if (m_mshr_stalled[core_id]) {
           m_mshr_stall_cycles.add2bin(gpu_sim_cycle + gpu_tot_sim_cycle - m_mshr_stall_cycle[core_id]);
	   m_mshr_stalled[core_id] = false;
       } 
   }
...
void ruby_wrapper::print_stats() const 
{
   ...
   m_mshr_stall_cycles.fprint(stdout);
   if(m_config.m_locality_profiler_enabled)
...
void ruby_wrapper::dump_request_buffer() {
    if (m_request_buffer_enabled) {
	FILE *outfile = fopen("mem_access_pattern.txt", "a+");
        while (!m_request_buffer.empty()) {
	    request_buf_entry buf_top = m_request_buffer.front();
            fprintf(outfile, "%-10d", buf_top.request_id);
            fprintf(outfile, "%-6d", buf_top.core_id);
            fprintf(outfile, "%-6d", buf_top.dynamic_cta_id);
            fprintf(outfile, "%-6d", buf_top.dynamic_warp_id);
            fprintf(outfile, "%-10s", buf_top.req_type.c_str());
            fprintf(outfile, "%-14d", buf_top.pc);
            fprintf(outfile, "%-18lld", buf_top.timestamp);
            fprintf(outfile, "%-18lld", buf_top.addr);
	    fprintf(outfile, "%-18lld", buf_top.block_addr);
            fprintf(outfile, "%-10s", buf_top.mshr_hit.c_str());
	    fprintf(outfile, "%-30s\n", buf_top.kernel_name.c_str());
	    m_request_buffer.pop();
	}
	fflush(outfile);
	fclose(outfile);
    } 
...
void ruby_wrapper::set_scheduler_stalled(unsigned core_id) {
    if (m_config.m_delay_queue_enabled)
        m_ruby->set_scheduler_stalled(core_id);
}

void ruby_wrapper::clear_scheduler_stalled(unsigned core_id) {
    if (m_config.m_delay_queue_enabled)
        m_ruby->clear_scheduler_stalled(core_id);
}

void ruby_wrapper::clear_delay_counters(unsigned core_id) {
    if (m_config.m_delay_queue_enabled)
        m_ruby->clear_delay_counters(core_id);
}
...
void ruby_stats::print(FILE *fout) const
{
   ...
   fprintf(fout, "ruby_stall[mshr_read_to_pending_write] = %u\n", m_stall_reasons[mshr_read_to_pending_write]);
}
...
bool ruby_mshr_table::access_allowed(ruby_addr_type block_addr, bool is_write, mem_fetch * mf, ruby_stall_reason_t& stall_reason) const {
    table::const_iterator i=m_data.find(block_addr);
    assert(i != m_data.end());      // there must be a pending request
    const entry& e = i->second;
    int warp_id = mf->get_wid();
    ...
    if(m_config.m_writes_stall_at_mshr) {
       ...
       if (m_config.m_atomics_stall_at_mshr) {
           if ((mf->get_is_write() && (e.m_writeset.size()==0 || e.m_has_atomic)) || mf->isatomic()) {
	      stall_reason = mshr_write_to_pending;
       ...
       */
       ...
       // Optional config. Stall reads if a pending write exists, and no other reads exist. If a read allocated the MSHR, then
       // more reads are okay to be merged in with it. This ensures write atomicity trivially.
       if(m_config.m_reads_stall_at_pending_writes and
               !is_write and e.m_writeset.count(warp_id)>0 and e.m_readset.count(warp_id)==0) {
           stall_reason = mshr_read_to_pending_write;
           return false;
       }
...
void ruby_mshr_table::add( ruby_addr_type block_addr, bool is_write, mem_fetch *mf )
{
    ...
    if(m_config.m_writes_stall_at_mshr) {
       ...
       if (m_config.m_atomics_stall_at_mshr && mf->isatomic()) {
	   assert(e.m_readset.size() == 0 and e.m_writeset.size() == 0);
           e.m_has_atomic = true;
       }
