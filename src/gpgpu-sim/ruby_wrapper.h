...
public:
    ...
    bool access_allowed(ruby_addr_type block_addr, bool is_write, mem_fetch * mf, ruby_stall_reason_t& stall_reason) const;
...
private:
	...
	bool m_has_atomic;             // atomic is recorded in mshr?
        mf_list m_list; 
        entry() : m_write_after_read(false), m_has_atomic(false) {}
    }; 
...
struct request_buf_entry {
    unsigned request_id;                //request_id
    unsigned core_id;                   //request core_id
    unsigned long long addr;            //address that this request want to access
    unsigned long long block_addr;      //cache block addr of this request to access
    unsigned long long timestamp;       //timestamp this request was issued
    unsigned pc;                        //program counter of this mem inst
    std::string req_type;               //request type (load, store, atomic)
    std::string mshr_hit;               //hit or miss MSHR
    unsigned dynamic_cta_id;            //request dynamic_cta_id 
    unsigned dynamic_warp_id;           //request dynamic_warp_id
    std::string kernel_name;            //name of the kernel function
};
...
public:
   ...
   bool is_ready( unsigned core_id, mem_fetch * mf, unsigned dynamic_cta_id, unsigned dynamic_warp_id, ruby_stall_type &m_ruby_stall_reason);
   void send_request( unsigned core_id, mem_fetch * mf, unsigned dynamic_cta_id, unsigned dynamic_warp_id, std::string kernel_name );
   void advance_time();
   ...
   void dump_request_buffer();

   //set and clear scheduler stalled bits in Ruby
   void set_scheduler_stalled(unsigned core_id);
   void clear_scheduler_stalled(unsigned core_id);
   void clear_delay_counters(unsigned core_id);
...
private:
   ...
   unsigned dynamic_request_id;
   bool m_request_buffer_enabled;
   unsigned m_request_buffer_size;
   std::queue<request_buf_entry> m_request_buffer;

   linear_histogram m_mshr_stall_cycles;
   std::vector<unsigned long> m_mshr_stall_cycle;
   std::vector<bool> m_mshr_stalled;
};
