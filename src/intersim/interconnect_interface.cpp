                        ...
                        unsigned int n_mem )
{
   ...
   for (unsigned i=0;i<net_c;i++) {
      ...
      if ( topo == "torus" ) {
         ...
         KNFly *knfly = new KNFly(icnt_config);
         if (knfly->GetK() * knfly->GetN() < _n_shader + _n_mem) {
            cerr << "ERROR: " << _n_shader << " shaders and " << _n_mem << " mem banks but interconnect is " << knfly->GetK() << "-ary " << knfly->GetN() << "-fly" << endl;
            exit(-1);
         }
         net[i] = knfly;
      } else if ( topo == "single" ) {
