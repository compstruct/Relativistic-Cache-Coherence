...
bool is_pred_tardisdec() {
    return CL_PREDICTOR_TYPE == CL_PREDICTOR_TYPE_TARDISDEC;
}
bool is_pred_tardisinc() {
    return CL_PREDICTOR_TYPE == CL_PREDICTOR_TYPE_TARDISINC;
}
bool is_pred_tardispcadaptive() {
    return CL_PREDICTOR_TYPE == CL_PREDICTOR_TYPE_TARDISPCADAPTIVE;
}
...
/*
 * TC TardisDec predictor functions
 */

Time get_pred_tardisdec_lifetime() {
    TCpredictorTardisDec* predictorTardisDec = dynamic_cast<TCpredictorTardisDec*>(g_system_ptr->getTCpredicterContainer()->getPredictor());
    return predictorTardisDec->get_pred_lifetime();
}

Time pred_tardisdec_load(Time pred_lease) {
    TCpredictorTardisDec* predictorTardisDec = dynamic_cast<TCpredictorTardisDec*>(g_system_ptr->getTCpredicterContainer()->getPredictor());
    return predictorTardisDec->update_lifetime_load(pred_lease);
}

Time pred_tardisdec_writeatomic(Time pred_lease) {
    TCpredictorTardisDec* predictorTardisDec = dynamic_cast<TCpredictorTardisDec*>(g_system_ptr->getTCpredicterContainer()->getPredictor());
    return predictorTardisDec->update_lifetime_writeatomic(pred_lease);
}

/*
 * TC TardisInc predictor functions
 */

Time get_pred_tardisinc_lifetime() {
    TCpredictorTardisInc* predictorTardisInc = dynamic_cast<TCpredictorTardisInc*>(g_system_ptr->getTCpredicterContainer()->getPredictor());
    return predictorTardisInc->get_pred_lifetime();
}

Time pred_tardisinc_load(Time pred_lease) {
    TCpredictorTardisInc* predictorTardisInc = dynamic_cast<TCpredictorTardisInc*>(g_system_ptr->getTCpredicterContainer()->getPredictor());
    return predictorTardisInc->update_lifetime_load(pred_lease);
}

Time pred_tardisinc_writeatomic(Time pred_lease) {
    TCpredictorTardisInc* predictorTardisInc = dynamic_cast<TCpredictorTardisInc*>(g_system_ptr->getTCpredicterContainer()->getPredictor());
    return predictorTardisInc->update_lifetime_writeatomic(pred_lease);
}

/*
 * TC TardisPCAdaptive predictor functions
 */

Time get_pred_tardispcadaptive_lifetime(int pc) {
    TCpredictorTardisPCAdaptive* predictorTardisPCAdaptive = dynamic_cast<TCpredictorTardisPCAdaptive*>(g_system_ptr->getTCpredicterContainer()->getPredictor());
    return predictorTardisPCAdaptive->get_pred_lifetime(pc);
}

void pred_tardispcadaptive_load(int pc) {
    TCpredictorTardisPCAdaptive* predictorTardisPCAdaptive = dynamic_cast<TCpredictorTardisPCAdaptive*>(g_system_ptr->getTCpredicterContainer()->getPredictor());
    predictorTardisPCAdaptive->update_lifetime_load(pc);
}

void pred_tardispcadaptive_writeatomic(int pc) {
    TCpredictorTardisPCAdaptive* predictorTardisPCAdaptive = dynamic_cast<TCpredictorTardisPCAdaptive*>(g_system_ptr->getTCpredicterContainer()->getPredictor());
    predictorTardisPCAdaptive->update_lifetime_writeatomic(pc);
}
