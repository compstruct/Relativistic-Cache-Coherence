...
TCpredictorContainer::TCpredictorContainer(int numL2Banks) {
    ...
    else if(CL_PREDICTOR_TYPE == CL_PREDICTOR_TYPE_TARDISDEC) 
	m_predictor = new TCpredictorTardisDec();
    else if(CL_PREDICTOR_TYPE == CL_PREDICTOR_TYPE_TARDISINC)
	m_predictor = new TCpredictorTardisInc();
    else if(CL_PREDICTOR_TYPE == CL_PREDICTOR_TYPE_TARDISPCADAPTIVE)
	m_predictor = new TCpredictorTardisPCAdaptive();
    else {
...
/*******************************************************************
 * TCpredictorTardisDec class
 *******************************************************************/
TCpredictorTardisDec::TCpredictorTardisDec() { 
    m_pred_min = (Time) CL_PRED_TARDISDEC_MIN;
    m_pred_max = (Time) CL_PRED_TARDISDEC_MAX;
    m_pred_shift = CL_PRED_TARDISDEC_SHIFT;
}

TCpredictorTardisDec::~TCpredictorTardisDec() { }

void TCpredictorTardisDec::print(ostream& out) const
{
    out << "[TCpredictorTardisDec]";
}

void TCpredictorTardisDec::printStats(ostream& out, bool short_stats)
{
    ...
    out << "TCpredictorTardisDec Stats" << endl;
    out << "--------------" << endl;
    out << "--------------" << endl;
}

void TCpredictorTardisDec::clearStats() { }

void TCpredictorTardisDec::printConfig(ostream& out) const
{

    out << endl;
    out << "TCpredictorTardisDec Configuration" << endl;
    out << "----------------------" << endl;
    out << "----------------------" << endl;
}

Time TCpredictorTardisDec::get_pred_lifetime() {
    return m_pred_max;
}

Time TCpredictorTardisDec::update_lifetime_load(Time pred_lease) {
    return (pred_lease << m_pred_shift) > m_pred_max ? m_pred_max : pred_lease << m_pred_shift;
}

Time TCpredictorTardisDec::update_lifetime_writeatomic(Time pred_lease) {
    return m_pred_min;
}

/*******************************************************************
 * TCpredictorTardisInc class
 *******************************************************************/
TCpredictorTardisInc::TCpredictorTardisInc() { 
    m_pred_min = (Time) CL_PRED_TARDISINC_MIN;
    m_pred_max = (Time) CL_PRED_TARDISINC_MAX;
    m_pred_shift = CL_PRED_TARDISINC_SHIFT;
}

TCpredictorTardisInc::~TCpredictorTardisInc() { }

void TCpredictorTardisInc::print(ostream& out) const
{
    out << "[TCpredictorTardisInc]";
}

void TCpredictorTardisInc::printStats(ostream& out, bool short_stats)
{
    ...
    out << "TCpredictorTardisInc Stats" << endl;
    out << "--------------" << endl;
    out << "--------------" << endl;
}

void TCpredictorTardisInc::clearStats() { }

void TCpredictorTardisInc::printConfig(ostream& out) const
{

    out << endl;
    out << "TCpredictorTardisInc Configuration" << endl;
    out << "----------------------" << endl;
    out << "----------------------" << endl;
}

Time TCpredictorTardisInc::get_pred_lifetime() {
    return m_pred_min;
}

Time TCpredictorTardisInc::update_lifetime_load(Time pred_lease) {
    return (pred_lease << m_pred_shift) > m_pred_max ? m_pred_max : pred_lease << m_pred_shift;
}

Time TCpredictorTardisInc::update_lifetime_writeatomic(Time pred_lease) {
    return m_pred_min;
}

/*******************************************************************
 * TCpredictorTardisPCAdaptive class
 *******************************************************************/
TCpredictorTardisPCAdaptive::TCpredictorTardisPCAdaptive() { 
    m_pred_min = CL_PRED_TARDISPCADAPTIVE_MIN;
    m_pred_max = CL_PRED_TARDISPCADAPTIVE_MAX;
    m_pred_shift = CL_PRED_TARDISPCADAPTIVE_SHIFT;
}

TCpredictorTardisPCAdaptive::~TCpredictorTardisPCAdaptive() { }

void TCpredictorTardisPCAdaptive::print(ostream& out) const
{
    out << "[TCpredictorTardisPCAdaptive]";
}

void TCpredictorTardisPCAdaptive::printStats(ostream& out, bool short_stats)
{
    ...
    out << "TCpredictorTardisPCAdaptive Stats" << endl;
    out << "--------------" << endl;
    out << "--------------" << endl;
}

void TCpredictorTardisPCAdaptive::clearStats() { }

void TCpredictorTardisPCAdaptive::printConfig(ostream& out) const
{

    out << endl;
    out << "TCpredictorTardisPCAdaptive Configuration" << endl;
    out << "----------------------" << endl;
    out << "----------------------" << endl;
}

Time TCpredictorTardisPCAdaptive::get_pred_lifetime(int pc) {
    assert(CL_PREDICTOR_TYPE == CL_PREDICTOR_TYPE_TARDISPCADAPTIVE);   // TardisPCAdaptive predictor

    if (m_leases.count(pc) == 0)
        m_leases[pc] = m_pred_min;
    
    Time return_lease = m_leases[pc];
    return return_lease;
}

void TCpredictorTardisPCAdaptive::update_lifetime_load(int pc) {
    assert(m_leases.count(pc) > 0);   // it must have requested a lease previously
    m_leases[pc] = m_leases[pc] << m_pred_shift;
    if( m_leases[pc] > m_pred_max)
        m_leases[pc] = m_pred_max;
}

void TCpredictorTardisPCAdaptive::update_lifetime_writeatomic(int pc) {
    assert(m_leases.count(pc) > 0);   // it must have requested a lease previously
    m_leases[pc] = m_leases[pc] >> m_pred_shift;
    if( m_leases[pc] < m_pred_min)
        m_leases[pc] = m_pred_min;
}
