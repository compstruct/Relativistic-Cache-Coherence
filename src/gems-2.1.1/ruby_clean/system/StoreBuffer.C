...
struct StoreBufferEntry {
  ...
  StoreBufferEntry(const SubBlock& block, CacheRequestType type, const Address& pc, AccessModeType access_mode, int size, int thread, int req_id, int warp_id, int cta_id) : m_subblock(block) {
    m_type = type;
    m_pc = pc;
  ...
  void print(ostream& out) const
  {
    out << "[StoreBufferEntry: " 
        ...
        << ", Size: " << m_size 
        << ", Thread: " << m_thread
	<< ", Dynamic_Req_ID: " << m_req
	<< ", Dynamic_Warp_ID: " << m_warp
	<< ", Dynamic_Cta_ID: " << m_cta
        << ", Time: " << m_time 
        << "]";
  }
...
void StoreBuffer::insertStore(const CacheMsg& request) 
{
  ...
  int warp_id = request.getWarpID();
  int cta_id = request.getCtaID();
  ...
  // Enqueue the entry
  StoreBufferEntry entry(subblock, type, pc, access_mode, size, threadID, request_id, warp_id, cta_id); // FIXME
  enqueue(entry);
...
void StoreBuffer::processHeadOfQueue()
{
  if(!isEmpty() && !m_pending) {
    ...
    CacheMsg request(entry.m_subblock.getAddress(), entry.m_subblock.getAddress(), entry.m_type, entry.m_pc, entry.m_access_mode, entry.m_size, PrefetchBit_No, 0, Address(0), entry.m_thread, entry.m_req, entry.m_warp, entry.m_cta, 0, false,MemorySpaceType_NULL,false,0,0);
