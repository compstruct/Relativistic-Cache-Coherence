...
public:
  // Constructors
  MessageBufferNode() { m_time = 0; m_msg_counter = 0; m_reply_wts = 0; m_delay_queue = false; }
  MessageBufferNode(const Time& time, int counter, const MsgPtr& msgptr, const Time& reply_wts = 0, bool delay_queue = false)
    { m_time = time; m_msgptr = msgptr; m_msg_counter = counter; m_reply_wts = reply_wts; m_delay_queue = delay_queue; }
  // Destructor
  //~MessageBufferNode();
...
{
  assert(n1.m_delay_queue == n2.m_delay_queue);
  if (n1.m_delay_queue == false) {
      ...
      if (n1.m_reply_wts == n2.m_reply_wts) {
        assert(n1.m_msg_counter != n2.m_msg_counter);
        return (n1.m_msg_counter <= n2.m_msg_counter);
      } else {
        return (n1.m_reply_wts <= n2.m_reply_wts);
      }
  }
}
