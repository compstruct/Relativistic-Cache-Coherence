...
struct gpusim_ruby_config {
    ...
    unsigned m_cl_pred_tardisdec_min;
    unsigned m_cl_pred_tardisdec_max;
    unsigned m_cl_pred_tardisdec_shift;
    unsigned m_cl_pred_tardisinc_min;
    unsigned m_cl_pred_tardisinc_max;
    unsigned m_cl_pred_tardisinc_shift;
    unsigned m_cl_pred_tardispcadaptive_min;
    unsigned m_cl_pred_tardispcadaptive_max;
    unsigned m_cl_pred_tardispcadaptive_shift;
    ...
    bool m_request_recorder_enabled;
    bool m_delay_queue_enabled;
    unsigned m_delay_queue_t_per_cycle;
    unsigned m_delay_queue_insert_latency;
    Time m_delay_queue_threshold;

    bool m_read_only_check;
    char * m_read_only_trace_file;
...
public:
   ...
   void send_request( unsigned long long addr, unsigned req_size, unsigned sid, unsigned tid, 
		      unsigned dynamic_cta_id, unsigned dynamic_warp_id, unsigned dynamic_request_id, unsigned pc,
                      ...
                      void* mf);
   void send_memfence_request( unsigned long long addr, unsigned req_size, unsigned sid, unsigned tid, 
		               unsigned dynamic_cta_id, unsigned dynamic_warp_id, unsigned dynamic_request_id, unsigned pc,
                               ruby_memory_type_t type, bool isPriv, ruby_memory_space_t space, ruby_mem_access_byte_mask_t access_mask);
   void advance_time();
   ...
   //set and clear scheduler stalled bits
