...
void gpusim_ruby::initialize() {
   ...
   NO_WRITE_ACKS = m_config.m_no_write_acks;
   ...
   CL_PRED_TARDISDEC_MIN = m_config.m_cl_pred_tardisdec_min;
   CL_PRED_TARDISDEC_MAX = m_config.m_cl_pred_tardisdec_max;
   CL_PRED_TARDISDEC_SHIFT = m_config.m_cl_pred_tardisdec_shift;
   CL_PRED_TARDISINC_MIN = m_config.m_cl_pred_tardisinc_min;
   CL_PRED_TARDISINC_MAX = m_config.m_cl_pred_tardisinc_max;
   CL_PRED_TARDISINC_SHIFT = m_config.m_cl_pred_tardisinc_shift;
   CL_PRED_TARDISPCADAPTIVE_MIN = m_config.m_cl_pred_tardispcadaptive_min;
   CL_PRED_TARDISPCADAPTIVE_MAX = m_config.m_cl_pred_tardispcadaptive_max;
   CL_PRED_TARDISPCADAPTIVE_SHIFT = m_config.m_cl_pred_tardispcadaptive_shift;
   ...
   g_REQUEST_RECORDER_ENABLED = m_config.m_request_recorder_enabled;
   g_DELAY_QUEUE_ENABLED = m_config.m_delay_queue_enabled;
   DELAY_QUEUE_TRANSITIONS_PER_RUBY_CYCLE = m_config.m_delay_queue_t_per_cycle;
   DELAY_QUEUE_INSERT_LATENCY = m_config.m_delay_queue_insert_latency;
   DELAY_QUEUE_THRESHOLD = m_config.m_delay_queue_threshold;

   READ_ONLY_CHECK = m_config.m_read_only_check;
   READ_ONLY_TRACE_FILE = m_config.m_read_only_trace_file;
...
void gpusim_ruby::send_request( unsigned long long addr, unsigned req_size, unsigned sid, unsigned tid, 
		                unsigned dynamic_cta_id, unsigned dynamic_warp_id, unsigned dynamic_request_id, unsigned pc,
                                ruby_memory_type_t type, bool isPriv, ruby_memory_space_t space, ruby_mem_access_byte_mask_t access_mask,
                                void* mf)
{
   GpusimInterface::makeRequest( addr, req_size, sid, tid, dynamic_cta_id, dynamic_warp_id, dynamic_request_id, pc, 
		                 getRubyMemType(type), isPriv, getRubyMemSpace(space), access_mask, mf );
}

  void gpusim_ruby::send_memfence_request( unsigned long long addr, unsigned req_size, unsigned sid, unsigned tid, 
		                           unsigned dynamic_cta_id, unsigned dynamic_warp_id, unsigned dynamic_request_id, unsigned pc,
                                           ruby_memory_type_t type, bool isPriv, ruby_memory_space_t space, ruby_mem_access_byte_mask_t access_mask)
  {
     GpusimInterface::makeMemfenceRequest( addr, req_size, sid, tid, dynamic_cta_id, dynamic_warp_id, dynamic_request_id, pc, 
		                           getRubyMemType(type), isPriv, getRubyMemSpace(space), access_mask );
  }
...
int gpusim_ruby::isReady( unsigned long long addr, unsigned req_size, unsigned sid, unsigned tid, 
		          unsigned dynamic_cta_id, unsigned dynamic_warp_id, unsigned dynamic_request_id, 
                          ruby_memory_type_t type, bool isPriv, ruby_memory_space_t space)
{
   return GpusimInterface::isReady(addr, req_size, sid, tid, dynamic_cta_id, dynamic_warp_id, dynamic_request_id, 
		                   getRubyMemType(type), getRubyMemSpace(space));
}
...
void gpusim_ruby::set_scheduler_stalled(unsigned core_id) { g_system_ptr->set_scheduler_stalled(core_id); }
void gpusim_ruby::clear_scheduler_stalled(unsigned core_id) { g_system_ptr->clear_scheduler_stalled(core_id); }
void gpusim_ruby::clear_delay_counters(unsigned core_id) { g_system_ptr->clear_delay_counters(core_id); }
