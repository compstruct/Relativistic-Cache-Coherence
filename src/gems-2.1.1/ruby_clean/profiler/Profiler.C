  ...
  : m_conflicting_histogram(-1)
{
  ...
  m_profiling_state = new Map<string, unsigned long>;
  m_profiling_lease = new Map<string, unsigned long>;
  m_profiling_request_miss = new Map<string, unsigned long>;
  m_profiling_bandwidth = new Map<string, unsigned long>;
  m_profiling_pts_changes = new Map<string, Histogram>;
  m_profiling_num_expired_blocks = new Map<string, Histogram>;
  m_profiling_expired_bandwidth = new Map<string, unsigned long>;
...
Profiler::~Profiler()
{
  ...
  delete m_profiling_state;
  delete m_profiling_lease;
  delete m_profiling_request_miss;
  delete m_profiling_pts_changes;
  delete m_profiling_num_expired_blocks;
  delete m_profiling_bandwidth;
  delete m_profiling_expired_bandwidth;
...
void Profiler::printStats(ostream& out, bool short_stats)
{
  ...
  out << endl;
  out << "Cache State" << endl;
  out << "----------------------" << endl;
  Vector<string> stateKeys = m_profiling_state->keys();
  for(int i=0; i<stateKeys.size(); i++) {
    unsigned long count = m_profiling_state->lookup(stateKeys[i]);
    out << stateKeys[i] << " = " << count << endl;
    total_states += count;
  }
  out << "----------------------" << endl;
  out << "total_states = " << total_states << endl;
  out << "----------------------" << endl;

  out << endl;
  unsigned long long total_leases = 0;
  out << endl;
  out << "Lease Distribution" << endl;
  out << "----------------------" << endl;
  Vector<string> leaseKeys = m_profiling_lease->keys();
  for(int i=0; i<leaseKeys.size(); i++) {
    unsigned long count = m_profiling_lease->lookup(leaseKeys[i]);
    out << leaseKeys[i] << " = " << count << endl;
    total_leases += count;
  }
  out << "----------------------" << endl;
  out << "total_leases = " << total_leases << endl;
  out << "----------------------" << endl;

  out << endl;
  Vector<string> requestMissKeys = m_profiling_request_miss->keys();
  for(int i=0; i<requestMissKeys.size(); i++) {
    unsigned long count = m_profiling_request_miss->lookup(requestMissKeys[i]);
    out << requestMissKeys[i] << " = " << count << endl;
  }
  
  out << endl;
  Vector<string> ptsChangeKeys = m_profiling_pts_changes->keys();
  for(int i=0; i<ptsChangeKeys.size(); i++) {
    Histogram hist = m_profiling_pts_changes->lookup(ptsChangeKeys[i]);
    out << ptsChangeKeys[i] << "_Pts_Change " << hist << endl;
  }
  
  out << endl;
  Vector<string> numExpiredBlocksKeys = m_profiling_num_expired_blocks->keys();
  for(int i=0; i<numExpiredBlocksKeys.size(); i++) {
    Histogram hist = m_profiling_num_expired_blocks->lookup(numExpiredBlocksKeys[i]);
    out << numExpiredBlocksKeys[i] << "_Num_Expired_Blocks " << hist << endl;
  }
  
  out << endl;
  out << "number_of_loads_before_replacement: " << m_profiling_number_loads << endl;
  ...
  out << endl;
  out << "Expired Read Breakdown" << endl;
  out << "----------------------" << endl;
  Vector<string> expiredReadKeys = m_profiling_expired_bandwidth->keys();
  for(int i=0; i<expiredReadKeys.size(); i++) {
    unsigned long count = m_profiling_expired_bandwidth->lookup(expiredReadKeys[i]);
    out << expiredReadKeys[i] << " = " << count << endl;
  }
...
void Profiler::clearStats()
{
  ...
  m_profiling_state->clear();
  
  m_profiling_state->add("STATE_NP", 0);
  m_profiling_state->add("STATE_P", 0);
  m_profiling_state->add("STATE_S", 0);
  m_profiling_state->add("STATE_M", 0);
  m_profiling_state->add("STATE_IS", 0);
  m_profiling_state->add("STATE_IAP", 0);
  
  m_profiling_lease->clear();
  
  m_profiling_lease->add("Lease_8", 0);
  m_profiling_lease->add("Lease_16", 0);
  m_profiling_lease->add("Lease_32", 0);
  m_profiling_lease->add("Lease_64", 0);
  m_profiling_lease->add("Lease_128", 0);
  m_profiling_lease->add("Lease_256", 0);
  m_profiling_lease->add("Lease_512", 0);
  m_profiling_lease->add("Lease_1024", 0);
  m_profiling_lease->add("Lease_2048", 0);
  m_profiling_lease->add("Lease_4096", 0);
  
  m_profiling_request_miss->clear();
  
  m_profiling_request_miss->add("Total_Miss", 0);
  m_profiling_request_miss->add("Expired_Miss", 0);

  m_profiling_pts_changes->clear();

  m_profiling_pts_changes->add("Load", Histogram(-1, 20));
  m_profiling_pts_changes->add("Store", Histogram(-1, 20));
  m_profiling_pts_changes->add("Atomic", Histogram(-1, 20));
  
  m_profiling_num_expired_blocks->clear();

  m_profiling_num_expired_blocks->add("Load", Histogram(-1, 20));
  m_profiling_num_expired_blocks->add("Store", Histogram(-1, 20));
  m_profiling_num_expired_blocks->add("Atomic", Histogram(-1, 20));
  m_profiling_num_expired_blocks->add("Fence", Histogram(-1, 20));
  ...
  m_profiling_bandwidth->add("L2_COHMSG_MESI", 0);
  m_profiling_bandwidth->add("L2_COHMSG_WT_MESI", 0);
  ...
  m_profiling_bandwidth->add("L1_COHMSG_MESI", 0);
  m_profiling_bandwidth->add("L1_COHMSG_WT_MESI", 0);

  m_profiling_bandwidth->add("L2_RENEW", 0);
  m_profiling_bandwidth->add("L1_DATA_TO_L2", 0);
  m_profiling_bandwidth->add("L1_ATOMIC_TO_L2", 0);
  m_profiling_bandwidth->add("L2_DATA_TO_L1", 0);
  m_profiling_bandwidth->add("L2_ATOMIC_TO_L1", 0);
  
  m_profiling_expired_bandwidth->clear();
  m_profiling_expired_bandwidth->add("EXPIRED_READ", 0);
  m_profiling_expired_bandwidth->add("EXPIRED_TOUCHED_READ", 0);
  m_profiling_expired_bandwidth->add("EXPIRED_UNTOUCHED_READ", 0);
  m_profiling_expired_bandwidth->add("OWNSELF_EXPIRED", 0);
  m_profiling_expired_bandwidth->add("TOUCHED_EXPIRED_BY_OWNSELF", 0);
  m_profiling_expired_bandwidth->add("UNTOUCHED_EXPIRED_BY_OWNSELF", 0);
  m_profiling_expired_bandwidth->add("OTHERS_EXPIRED", 0);
  m_profiling_expired_bandwidth->add("TOUCHED_EXPIRED_BY_OTHERS", 0);
  m_profiling_expired_bandwidth->add("UNTOUCHED_EXPIRED_BY_OTHERS", 0);
  m_profiling_expired_bandwidth->add("EXPIRED_RENEW", 0);
  m_profiling_expired_bandwidth->add("EXPIRED_OWN_WARP", 0);
  m_profiling_expired_bandwidth->add("EXPIRED_SHARED_WARP", 0);
  m_profiling_expired_bandwidth->add("EXPIRED_OTHER_WARP", 0);
  m_profiling_expired_bandwidth->add("EXPIRED_SHARED_CTA", 0);
  m_profiling_expired_bandwidth->add("EXPIRED_OTHER_CTA", 0);
  ...
  m_profiling_number_loads.clear();
...
void Profiler::profileState(string type, int count) {
   assert(m_profiling_state->exist(type));
   (m_profiling_state->lookup(type)) += count;
}

void Profiler::profileLease(string type, int count) {
   assert(m_profiling_lease->exist(type));
   (m_profiling_lease->lookup(type)) += count;
}

void Profiler::profileRequestMiss(string type) {
   assert(m_profiling_request_miss->exist(type));
   (m_profiling_request_miss->lookup(type))++;
}

void Profiler::profilePtsChange(string type, Time pts_change) {
    assert(m_profiling_pts_changes->exist(type));
    m_profiling_pts_changes->lookup(type).add(pts_change);
}

void Profiler::profileNumExpiredBlocks(string type, int count) {
    assert(m_profiling_num_expired_blocks->exist(type));
    m_profiling_num_expired_blocks->lookup(type).add(count);
}
...
void Profiler::profileExpiredBandwidth(string type, MessageSizeType size) {
   assert(m_profiling_expired_bandwidth->exist(type));
   int num_flits = (int) ceil((double) MessageSizeType_to_int(size)/NetworkConfig::getFlitSize() );
   (m_profiling_expired_bandwidth->lookup(type)) += num_flits;
}
...
void Profiler::profileNumberOfLoads(int num_loads)
{
  m_profiling_number_loads.add(num_loads);
}
